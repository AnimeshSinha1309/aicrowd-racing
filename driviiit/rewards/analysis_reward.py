...
class AnalyzerReward(GranTurismo):
    ...
    def get_reward(self, state, oob_flag=False):
        ...
        speed_along_road, speed_across_road = self._get_velocity_components(race_idx, velocity)
    ...
    def _get_velocity_components(self, race_idx, velocity):
        velocity = velocity[:2]
        road_direction = self.centre_path[(race_idx + 1) % len(self.centre_path)] - self.centre_path[race_idx]
