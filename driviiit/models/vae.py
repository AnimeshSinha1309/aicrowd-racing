...
class VAE(torch.nn.Module):
    ...
    def __init__(self, im_c=3, im_h=384, im_w=512, z_dim=32):
        ...
        self.decoder = torch.nn.Sequential(
            ...
            torch.nn.ConvTranspose2d(em_shape[0], 128, kernel_size=(4, 4), stride=(2,), padding=(1, 1)),
            torch.nn.ReLU(),
            torch.nn.ConvTranspose2d(128, 64, kernel_size=(4, 4), stride=(2,), padding=(1, 1)),
            torch.nn.ReLU(),
            torch.nn.ConvTranspose2d(64, 32, kernel_size=(4, 4), stride=(2,), padding=(1, 1)),
            torch.nn.ReLU(),
            torch.nn.ConvTranspose2d(32, im_c, kernel_size=(4, 4), stride=(2,), padding=(1, 1)),
            torch.nn.Sigmoid(),
        )
    ...
    @staticmethod
    def loss(actual, reconstructed, mu, log_variable, kld_weight=1.):
        bce = torch.nn.functional.binary_cross_entropy(reconstructed, actual, reduction='sum')
...
if __name__ == '__main__':
    ...
    visualize_indices = np.concatenate([np.random.choice(train_indices, 5), np.random.choice(train_indices, 15)])
    ...
    def evaluate(epoch):
        ...
        test_loss = []
        vae.eval()
        with tqdm.trange(len(test_indices) // batch_size, desc=f"Epoch #{epoch} test") as iterator:
            for i in iterator:
                index = test_indices[batch_size * i: batch_size * (i + 1)]
                img = torch.as_tensor(images[index] / 255., device=DEVICE, dtype=torch.float)
        ...
        for idx in visualize_indices:
            original_image = torch.as_tensor(images[idx] / 255., device=DEVICE, dtype=torch.float)
            reconstructed_image = vae(original_image[None])[0][0].detach().cpu().numpy()
            wandb.log({
                'vae/original_image': wandb.Image(images[idx].transpose(1, 2, 0), caption=f"{idx}-original"),
                'vae/reconstructed_image': wandb.Image(reconstructed_image.transpose(1, 2, 0), caption=f"{idx}-vae"),
                'epoch': epoch
            })
    ...
    def train(epoch):
        ...
        train_loss = []
        vae.train()
        with tqdm.trange(len(train_indices) // batch_size, desc=f"Epoch #{epoch} train") as iterator:
            for i in iterator:
